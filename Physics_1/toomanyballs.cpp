...
int main(){
    Body body_1 = {Vec2 {100,300},Vec2 {100,0},Vec2 {0,9.8},2};
    Body body_2 = {Vec2 {0,200},Vec2 {10,0},Vec2 {0,9.8},2};
    Body body_3 = {Vec2 {400,400},Vec2 {100,0},Vec2 {0,9.8},2};
    Body body_4 = {Vec2 {400,200},Vec2 {10,0},Vec2 {0,9.8},2};
    
    vector<Body*> bodies;
    
    vector<vector<Vec2*>> rates;
    
    bodies.push_back(& body_1);
    bodies.push_back(& body_2);
    bodies.push_back(& body_3);
    bodies.push_back(& body_4);
    for(Body* body : bodies){

        rates.push_back({& (*body).acceleration,& (*body).velocity}); 
    ...
    while(window.isOpen()){
        collision(bodies, 100);
        if (body_1.position.y>500 or body_1.position.y<0){
            
            body_1.velocity.y =-body_1.velocity.y;
            body_1.position.y = prev_1[0].y;
        }
        
        if (body_2.position.y>500 or body_2.position.y<0){
            
            body_2.velocity.y =-body_2.velocity.y;
            body_2.position.y = prev_2[0].y;
        }
        
        if (body_1.position.x>700 or body_1.position.x<0){
            body_1.velocity.x =-body_1.velocity.x;
            body_1.position.x = prev_1[0].x;
        }
        if (body_2.position.x>700 or body_2.position.x<0){
        ...
        integrate(body_1.position,rates[0],prev_1);
        integrate(body_2.position,rates[1],prev_2);
